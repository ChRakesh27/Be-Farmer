  ...
  onSelectImg(e: any) {
    if (e.target.files) {
      ...
      reader.onload = (event: any) => {
        ...
        // this.userform.patchValue({ profileImg: this.imageBase64 });
        // console.log(this.userform.value.profileImg)
      }
    }
  }
  ...
  login(email: any, password: any) {
    this.service.LoginDel(email, password).subscribe((res) => {
      if (res.data[0] !== undefined) {
        let data = { id: res.data[0].id, username: res.data[0].username, email: res.data[0].email, phone: res.data[0].phone, password: res.data[0].password, profileImg: res.data[0].profileImg };
        ...
        // this.imageBase64 = res.data[0].profileImg;
      }
    })
  }
